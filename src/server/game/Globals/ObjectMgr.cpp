...
void ObjectMgr::LoadCreatureTemplate(Field* fields)
{
    ...
    creatureTemplate.CreatureImmunitiesId   = fields[42].GetInt32();
    creatureTemplate.flags_extra            = fields[43].GetUInt32();
    creatureTemplate.ScriptID               = GetScriptId(fields[44].GetString());
    creatureTemplate.StringId               = fields[45].GetString();
}
...
void ObjectMgr::LoadCreatureTemplateDifficulty()
{
    ...
    //                                               0      1             2                     3                     4                5
    QueryResult result = WorldDatabase.Query("SELECT Entry, DifficultyID, LevelScalingDeltaMin, LevelScalingDeltaMax, SandboxScalingId, HealthScalingExpansion, "
    //   6               7             8              9               10                    11         12
    ...
    do
    {
        ...
        creatureDifficulty.SandboxScalingId        = fields[4].GetInt32();
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    for (ItemEffectEntry const* effectEntry : sItemEffectStore)
        if (ItemTemplate* item = Trinity::Containers::MapGetValuePtr(_itemTemplateStore, effectEntry->ParentItemID))
            item->Effects.push_back(effectEntry);
                    ...
                    if (raceMask.IsEmpty() || raceMask.HasRace(raceIndex))
                    {
                        for (uint32 classIndex = CLASS_WARRIOR; classIndex < MAX_CLASSES; ++classIndex)
                        {
                            if (classMask == 0 || ((1 << (classIndex - 1)) & classMask))
                            {
                                if (auto const& playerInfo = Trinity::Containers::MapGetValuePtr(_playerInfo, { Races(raceIndex), Classes(classIndex) }))
                                {
                                    playerInfo->get()->castSpells.push_back(spellId);
                                    ++count;
                                }
                            }
                        }
...
void ObjectMgr::LoadEventSet()
{
    ...
    for (SpellEntry const* spellEntry : sSpellStore)
        if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(spellEntry->ID, DIFFICULTY_NONE))
