...
void Player::UpdateCritPercentage(WeaponAttackType attType)
{
    ...
    {
        case OFF_ATTACK:
            SetStatFloatValue(PLAYER_OFFHAND_CRIT_PERCENTAGE, applyCritLimit(GetBaseModValue(OFFHAND_CRIT_PERCENTAGE, FLAT_MOD) + GetBaseModValue(OFFHAND_CRIT_PERCENTAGE, PCT_MOD) + GetRatingBonusValue(CR_CRIT_MELEE)));
            break;
        case RANGED_ATTACK:
            SetStatFloatValue(PLAYER_RANGED_CRIT_PERCENTAGE, applyCritLimit(GetBaseModValue(RANGED_CRIT_PERCENTAGE, FLAT_MOD) + GetBaseModValue(RANGED_CRIT_PERCENTAGE, PCT_MOD) + GetRatingBonusValue(CR_CRIT_RANGED)));
            break;
        case BASE_ATTACK:
        default:
            SetStatFloatValue(PLAYER_CRIT_PERCENTAGE, applyCritLimit(GetBaseModValue(CRIT_PERCENTAGE, FLAT_MOD) + GetBaseModValue(CRIT_PERCENTAGE, PCT_MOD) + GetRatingBonusValue(CR_CRIT_MELEE)));
            break;
    }
}
...
void Player::UpdateVersatilityDamageDone()
{
    ...
    SetFloatValue(PLAYER_VERSATILITY, m_baseRatingValue[CR_VERSATILITY_DAMAGE_DONE]);
...
void Player::UpdateHealingDonePercentMod()
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_SCHOOL; ++i)
        SetFloatValue(PLAYER_FIELD_MOD_HEALING_DONE_POS + 1, value);
}
...
{
    // Store Rating Value
    SetUInt32Value(AsUnderlyingType(PLAYER_FIELD_COMBAT_RATING_1) + CR_ARMOR_PENETRATION, amount);
}
...
void Player::UpdateAllRunesRegen()
{
    ...
    SetStatFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER + runeIndex, float(1 * IN_MILLISECONDS) / float(cooldown) - runeEntry->RegenPeace);
}
...
void Guardian::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (owner)
    {
        ...
        {
            int32 fire  = int32(owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_FIRE)) + owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG) + SPELL_SCHOOL_FIRE);
            int32 shadow = int32(owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_SHADOW)) + owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG) + SPELL_SCHOOL_SHADOW);
        ...
        {
            int32 frost = int32(owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_FROST)) + owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG) + SPELL_SCHOOL_FROST);
            if (frost < 0)
                frost = 0;
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    if (Player* playerOwner = m_owner->ToPlayer())
    {
        ...
        {
            int32 spellDmg = int32(m_owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_NATURE)) + m_owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG) + SPELL_SCHOOL_NATURE);
            if (spellDmg > 0)
        ...
        {
            int32 spellDmg = int32(m_owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_FIRE)) + m_owner->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG) + SPELL_SCHOOL_FIRE);
            if (spellDmg > 0)
