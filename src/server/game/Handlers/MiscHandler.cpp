...
void WorldSession::HandleAreaTriggerOpcode(WorldPackets::AreaTrigger::AreaTrigger& packet)
{
    ...
    if (!teleported)
    {
        ...
        if (entranceLocation && player->GetMapId() != at->target_mapId)
            ...
            player->TeleportTo(entranceLocation->MapID, entranceLocation->GetPositionX(), entranceLocation->GetPositionY(),entranceLocation->GetPositionZ(), TELE_TO_NOT_LEAVE_TRANSPORT);
        else
