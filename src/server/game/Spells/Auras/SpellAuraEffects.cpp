...
m_baseAmount(baseAmount ? *baseAmount : spellEfffectInfo.CalcBaseValue(caster, base->GetType() == UNIT_AURA_TYPE ? base->GetOwner()->ToUnit() : nullptr, base->GetCastItemLevel())),
...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    if (!m_spellInfo->HasAttribute(SPELL_ATTR8_MASTERY_AFFECTS_POINTS) || G3D::fuzzyEq(GetSpellEffectInfo().BonusCoefficient, 0.0f))
        ...
        amount = int32(caster->ToPlayer()->GetFloatValue(PLAYER_MASTERY) * GetSpellEffectInfo().BonusCoefficient);
                                                         ...
                                                         float amount, uint8 stack)
{
    ...
    if (spellInfo->IsChanneled())
        ...
        period = int32(period * caster->GetModCastingSpeed());
    else if (spellInfo->HasAttribute(SPELL_ATTR8_MELEE_HASTE_AFFECTS_PERIODIC))
        period = int32(period * caster->GetFloatValue(UNIT_FIELD_MOD_HASTE));
...
void AuraEffect::PeriodicTick(AuraApplication* aurApp, Unit* caster) const
{
    ...
    if (caster && !caster->IsPlayer() && m_spellInfo->IsChanneled() && m_spellInfo->HasAttribute(SPELL_ATTR1_TRACK_TARGET_IN_CHANNEL) && caster->GetChannelObjects().size() > 0)
    {
