    ...
    private:
        ...
        void CalculateAttackPowerAmount(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* pet = GetUnitOwner())
                ...
                {
                    int32 fire  = owner->GetInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_FIRE) - owner->GetInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG) + SPELL_SCHOOL_FIRE);
                    int32 shadow = owner->GetInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_SHADOW) - owner->GetInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG) + SPELL_SCHOOL_SHADOW);
