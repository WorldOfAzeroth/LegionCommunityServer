...
#define SPELL_UNSTUCK_VISUAL 2683
    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (banTime >= 0)
            handler->PSendSysMessage(LANG_PINFO_BANNED, banType.c_str(), banReason.c_str(), banTime > 0 ? secsToTimeString(banTime - time(NULL), TimeFormat::ShortText).c_str() : handler->GetTrinityString(LANG_PERMANENTLY), bannedBy.c_str());

        // Output IV. LANG_PINFO_MUTED if mute is applied
        if (muteTime > 0)
            handler->PSendSysMessage(LANG_PINFO_MUTED, muteReason.c_str(), secsToTimeString(muteTime - time(nullptr), TimeFormat::ShortText).c_str(), muteBy.c_str());
