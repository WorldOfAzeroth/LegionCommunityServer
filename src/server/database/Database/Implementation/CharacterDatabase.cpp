...
#define SelectItemInstanceContent "ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, 0, ii.randomBonusListId, " \
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_QUESTSTATUS, "SELECT quest, status, explored, endTime FROM character_queststatus WHERE guid = ? AND status <> 0", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHAR_QUESTSTATUS, "REPLACE INTO character_queststatus (guid, quest, status, explored, endTime) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
